...
# ==========================================
# 📄 ENTERPRISE PDF ENGINE (ENHANCED)
# ==========================================
...
def generate_pdf(client, income, df_main_results, is_pass, exposure, shortfall, mode, active_s_name, active_s_rate, active_s_inc, raw_loans, all_scenarios_data, agg_dti, stressed_sources_list=None, gross_income=None, inc_mode=None):
    """
    Enhanced PDF generator with comprehensive scenario analysis
    
    all_scenarios_data: List of dicts with scenario analysis results
        [{'name': str, 'rate': float, 'income': float, 'stressed_income': float, 
          'total_obligation': float, 'agg_coverage': float, 'pass': bool, 'df': DataFrame}, ...]
    """
    pdf = PDFReport()
    pdf.add_page()
    ...
    # ==========================================
    if all_scenarios_data and len(all_scenarios_data) > 0:
        pdf.add_page()
        pdf.set_font("Arial", "B", 13)
        pdf.set_text_color(15, 23, 42)
        pdf.cell(0, 8, "COMPREHENSIVE STRESS TEST MATRIX", 0, 1)
        pdf.set_draw_color(226, 232, 240)
        pdf.line(10, pdf.get_y(), 200, pdf.get_y())
        pdf.ln(6)
        
        # Table headers
        cols = [50, 22, 22, 28, 28, 22, 18]
        headers = ["Scenario", "Rate", "Income", "Stressed Inc.", "Tot. Oblig.", "Agg. Cov.", "Result"]
        
        pdf.set_font("Arial", "B", 7)
        pdf.set_fill_color(15, 23, 42)
        pdf.set_text_color(255, 255, 255)
        for i, h in enumerate(headers):
            pdf.cell(cols[i], 7, h, 1, 0, 'C', fill=True)
        pdf.ln()
        pdf.set_text_color(0, 0, 0)
        
        # Scenario rows
        pdf.set_font("Arial", "", 7)
        for idx, scenario in enumerate(all_scenarios_data):
            fill = (idx % 2 == 0)
            pdf.set_fill_color(255, 255, 255) if not fill else pdf.set_fill_color(248, 250, 252)
            
            # Truncate scenario name if too long
            name = scenario['name'][:28] if len(scenario['name']) > 28 else scenario['name']
            pdf.cell(cols[0], 6, name, 1, 0, 'L', fill)
            pdf.cell(cols[1], 6, f"+{scenario['rate']:.2f}%", 1, 0, 'C', fill)
            pdf.cell(cols[2], 6, f"-{scenario['income']:.2f}%", 1, 0, 'C', fill)
            pdf.cell(cols[3], 6, f"Rs.{scenario['stressed_income']:,.0f}", 1, 0, 'R', fill)
            pdf.cell(cols[4], 6, f"Rs.{scenario['total_obligation']:,.0f}", 1, 0, 'R', fill)
            pdf.cell(cols[5], 6, f"{scenario['agg_coverage']:.2f}x", 1, 0, 'C', fill)
            ...
            pdf.cell(cols[6], 6, result, 1, 1, 'C', fill)
            pdf.set_text_color(0, 0, 0)
        ...
        pdf.multi_cell(0, 4, 
            "Note: This matrix shows portfolio performance across all defined stress scenarios. "
            "Each scenario applies different combinations of interest rate shocks and income reductions "
            "to test portfolio resilience under adverse conditions.")
        pdf.set_text_color(0, 0, 0)

    # PRIORITY ALLOCATION BREAKDOWN (Active Scenario)
    pdf.add_page()
    pdf.set_font("Arial", "B", 12)
    pdf.set_text_color(15, 23, 42)
    ...
    pdf.set_text_color(100, 116, 139)
    pdf.cell(0, 6, f"Active Scenario: {active_s_name}", 0, 1)
    pdf.set_text_color(0, 0, 0)
    pdf.ln(2)
    ...
    # ==========================================
    if all_scenarios_data and len(all_scenarios_data) > 1:
        for scenario in all_scenarios_data:
            # Skip the active scenario (already shown above)
            if scenario['name'] == active_s_name:
                continue
                
            pdf.add_page()
            pdf.set_font("Arial", "B", 12)
            pdf.set_text_color(15, 23, 42)
            pdf.cell(0, 8, "SCENARIO DETAILED BREAKDOWN", 0, 1)
            pdf.set_draw_color(226, 232, 240)
            pdf.line(10, pdf.get_y(), 200, pdf.get_y())
            pdf.ln(4)
            ...
            pdf.set_fill_color(219, 234, 254)
            pdf.set_font("Arial", "B", 10)
            pdf.cell(190, 7, f"  Scenario: {scenario['name']}", 1, 1, 'L', fill=True)
            
            pdf.set_font("Arial", "", 9)
            pdf.set_fill_color(248, 250, 252)
            pdf.cell(95, 6, f"Interest Rate Shock: +{scenario['rate']:.2f}%", 1, 0, 'L', fill=True)
            pdf.cell(95, 6, f"Income Reduction: -{scenario['income']:.2f}%", 1, 1, 'L', fill=True)
            
            pdf.cell(95, 6, f"Stressed Income: Rs.{scenario['stressed_income']:,.2f}", 1, 0, 'L', fill=True)
            pdf.cell(95, 6, f"Aggregate Coverage: {scenario['agg_coverage']:.2f}x", 1, 1, 'L', fill=True)
            
            pdf.ln(2)
            result_txt = "APPROVED - Within Tolerance" if scenario['pass'] else "DECLINED - Exceeds Limits"
            pdf.set_text_color(16, 185, 129) if scenario['pass'] else pdf.set_text_color(239, 68, 68)
            pdf.set_font("Arial", "B", 10)
            pdf.cell(0, 7, f"Result: {result_txt}", 0, 1)
            pdf.set_text_color(0, 0, 0)
            pdf.ln(4)
            ...
            for idx, row in df_scenario.iterrows():
                ...
                pdf.cell(cols[2], 7, f"{row['Obligation']:,.0f}", 1, 0, 'R', fill)
                pdf.cell(cols[3], 7, f"{row['Available_Income_Snapshot']:,.0f}", 1, 0, 'R', fill)
                pdf.cell(cols[4], 7, f"{row['Actual Coverage']:.2f}x", 1, 0, 'C', fill)
                pdf.cell(cols[5], 7, f"{row['Required Multiplier']:.2f}x", 1, 0, 'C', fill)
            ...
            if st.button("🚀 Generate PDF", type="primary", width='stretch'):
                if not report_name:
                    ...
                    with st.spinner("Processing document..."):
                        # Prepare all scenarios data for PDF
                        all_scenarios_data = []
                        
                        # Add baseline scenario
                        base_income = gross_income
                        base_loans = pd.DataFrame(st.session_state.loans)
                        base_loans[['Obligation', 'Effective_Rate']] = base_loans.apply(
                            lambda x: pd.Series(get_stress_row(x, 0.0)), axis=1
                        )
                        base_res = run_waterfall_allocation(base_loans, base_income)
                        base_obl = base_res['Obligation'].sum()
                        base_cov = base_income / base_obl if base_obl > 0 else 0
                        ...
                        all_scenarios_data.append({
                            ...
                            'rate': 0.0,
                            'income': 0.0,
                            'stressed_income': base_income,
                            'total_obligation': base_obl,
                            'agg_coverage': base_cov,
                            'pass': base_pass,
                            'df': base_res
                        })
                        
                        # Add all custom scenarios
                        if enable_stress and st.session_state.custom_scenarios:
                            for scen in st.session_state.custom_scenarios:
                                s_name = scen['Name']
                                s_rate = scen['Rate']
                                s_inc_pct = scen['Income']
                                
                                # Calculate stressed income
                                scen_income = 0.0
                                if inc_mode == "Multiple Sources" and stressed_sources_selection:
                                    v_inc = sum(x['Amount'] for x in st.session_state.income_sources 
                                ...
                                # Calculate stressed obligations
                                temp_df = pd.DataFrame(st.session_state.loans)
                                temp_df[['Obligation', 'Effective_Rate']] = temp_df.apply(
                                    lambda x: pd.Series(get_stress_row(x, s_rate)), axis=1
                                )
                                scen_res = run_waterfall_allocation(temp_df, scen_income)
                                scen_tot_obl = scen_res['Obligation'].sum()
                                scen_agg = scen_income / scen_tot_obl if scen_tot_obl > 0 else 0
                                ...
                                all_scenarios_data.append({
                                    ...
                                    'rate': s_rate,
                                    'income': s_inc_pct,
                                    'stressed_income': scen_income,
                                    'total_obligation': scen_tot_obl,
                                    'agg_coverage': scen_agg,
                                    'pass': scen_pass,
                                    'df': scen_res
                                })
                        ...
                        pdf_bytes = generate_pdf(
                            ...
                            st.session_state.loans, all_scenarios_data, agg_dti, sources_for_pdf,
                            gross_income, inc_mode
                        )
